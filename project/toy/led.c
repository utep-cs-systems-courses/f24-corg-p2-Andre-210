...
unsigned char r_on = 0, g_on = 0;       // initially off (0)
...
void led_update() {
  ...
  if (led_altered) {
    char ledFlags = redVal[r_on] | greenVal[g_on]; // find the desired state for the leds (0 or 1)
...
void green_on() {
  P1OUT |= LED_GREEN;             // set bit for the green led (turning it on)
  __delay_cycles(500000);         // delay of 500000 clock cycles
}

// turns on the red led
void red_on() {
  P1DIR |= LED_RED;               // set bit for the red led (turning it on)
  __delay_cycles(500000);         // delay of 500000 clock cycles
}
