...
// function prototypes
void led_init();              // initializes led hardware
void led_update();            // updates the led status based on r_on/g_on flags
void green_on();              // turns on the green led
void red_on();                // turns on the red led
void leds_off();              // turns off both leds
void led_flash(int n);        // flashes the leds

// flags to check states
extern unsigned char r_on, g_on;          // indicates states of red and green leds
